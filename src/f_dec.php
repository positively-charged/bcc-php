      ...
      if ( ! $func ) {
         if ( $front->tk == tk_lit_decimal ) {
            ...
            if ( $dec[ 'storage' ] == k_storage_global ) {
               ...
               f_diag( $front, k_diag_err | k_diag_file | k_diag_line |
                  k_diag_column, $pos, 'index specified for %s storage',
                  $dec[ 'storage_name' ] );
               f_bail( $front );
            }
...
function f_read_script( $front ) {
   ...
   if ( $front->tk == tk_shift_l ) {
      ...
      if ( $expr->value < k_script_min_num ||
   ...
   if ( $front->tk == tk_paren_l ) {
      $script->params_pos = $front->tk_pos;
      f_read_tk( $front );
      $script->params_num = f_read_param_list( $front, true );
      f_test_tk( $front, tk_paren_r );
      f_read_tk( $front );
   }
   // Script type.
   ...
   case script_t::type_closed:
      if ( $script->params_num > k_script_max_params ) {
         f_diag( $front, k_diag_err | k_diag_file | k_diag_line |
            k_diag_column, $script->params_pos,
